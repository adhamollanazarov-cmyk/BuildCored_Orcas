...
def is_register(s):
    return isinstance(s, str) and s.startswith("R") and len(s) >= 2 and s[1:].isdigit()
...
def execute(cpu, instruction):
    ...
    if op == "MOV":
        ...
        cpu.registers[dst] = alu("ADD", left, right)
        cpu.pc += 1
        return f"R{dst} = {left} + {right} = {cpu.registers[dst]}"

    elif op == "SUB":
        dst = reg_index(args[0])
        left = cpu.registers[dst]
        right = get_value(cpu, args[1])
        cpu.registers[dst] = alu("SUB", left, right)
        cpu.pc += 1
        return f"R{dst} = {left} - {right} = {cpu.registers[dst]}"
        ...
        left = cpu.registers[dst]
        right = get_value(cpu, args[1])
        cpu.registers[dst] = alu("MUL", left, right)
        cpu.pc += 1
        return f"R{dst} = {left} * {right} = {cpu.registers[dst]}"
...
# ====== TODO #2: Tuned narrator prompt ======
NARRATOR_PROMPT = """You are a computer architecture tutor.

Explain this CPU step in one short, clear, educational sentence.
Focus on registers, flags, ALU, or control flow.
Keep it simple for a beginner.
Do not repeat the instruction literally.
...
def show_state(cpu, step, instr, effect, narration):
    t = Table(title=f"Step {step}: {' '.join(map(str, instr))}", show_header=True)
    for i in range(8):
...
while not cpu.halted and cpu.pc < len(cpu.program) and step < 50:
    ...
    regs_str = ", ".join(f"R{i}={v}" for i, v in enumerate(cpu.registers))
