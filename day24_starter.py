...
def get_sample_datasheets():
    pdfs = [
        f for f in os.listdir(DATASHEET_DIR)
...
Sampling rate: up to 100K samples/second.
    ...
    for title, content in content_pages:
        ...
        page.insert_text((50, 90), content.strip(), fontsize=10, color=(0, 0, 0))
...
def retrieve(collection, embedder, query, n_results=5):
...
def generate_answer(query, retrieved_results):
    ...
    prompt = f"""You are a hardware engineering assistant.

Answer the question using ONLY the provided datasheet excerpts.
Cite the source and page number for every important value or specification.
If the answer is not available in the excerpts, say that the datasheet excerpts do not provide enough information.
...
Answer concisely with specific values/specs where available.
Always cite like this: [Source N, Page X]
"""
...
CROSS_DOCUMENT_QUESTION = (
    "Compare the ADC resolution, I2C support, GPIO current, "
    "and operating temperature across the available datasheets."
)
